...
public final class PhantomCommands {
    ...
    private static boolean requireDeveloperFeatures(com.mojang.brigadier.context.CommandContext<FabricClientCommandSource> ctx) {
        if (PhantomConfig.isDeveloperFeaturesEnabled()) return true;
                        ...
                        ClientCommands.literal("phantom")
                                ...
                                .then(ClientCommands.literal("force")
                                        .then(ClientCommands.literal("tracker")
                                                .executes(ctx -> {
                                                    if (!requireDeveloperFeatures(ctx)) return 0;
                                                    boolean on = PhantomConfig.toggleProfitTrackerForced();
                                                    Minecraft mc = Minecraft.getInstance();
                                                    if (mc.player != null) {
                                                        mc.player.sendSystemMessage(Component.literal(
                                ...
                                .then(ClientCommands.literal("setpb")
                                        ...
                                        .then(ClientCommands.argument("tier",
                                                ...
                                                .executes(ctx -> {
                                                    openPbEditor(IntegerArgumentType.getInteger(ctx, "tier"));
                                                    return 1;
                                                })
    ...
    private static void forceRemoveRootChild(CommandDispatcher<FabricClientCommandSource> dispatcher, String name) {
        try {
            ...
            PhantomAddons.LOGGER.warn("[PhantomCommands] Failed to force-clear conflicting '{}' command node: {}", name, e.toString());
        }
    }
    ...
    private static Component buildTooltip(int tier, PhantomConfig.PbRecord record) {
        MutableComponent t = Component.literal("§e" + TIER_NAMES[tier] + " Tier PB:\n");
        t.append(Component.literal("§fTime: §a" + PhantomConfig.formatTime(record.totalTime) + "\n"));
        ...
        if (record.splits != null && record.splits.length == 7) {
            ...
            for (int i = 0; i < 7; i++) {
                String val = record.splits[i] < 9999
                        ? PhantomConfig.formatTime(record.splits[i]) : "—";
        ...
        if (record.supplies != null && !record.supplies.isEmpty()) {
            t.append(Component.literal("\n§eSupplies:\n"));
            List<PhantomConfig.PlayerTime> sorted = new ArrayList<>(record.supplies);
            sorted.sort((a, b) -> Double.compare(a.time, b.time));
            for (int i = 0; i < sorted.size(); i++) {
                PhantomConfig.PlayerTime pt = sorted.get(i);
                t.append(Component.literal("§7 #" + (i + 1) + " §f" + pt.player
                        + " §7" + PhantomConfig.formatTime(pt.time) + "\n"));
            }
        }

        if (record.freshes != null && !record.freshes.isEmpty()) {
            t.append(Component.literal("\n§eFresh:\n"));
            List<PhantomConfig.PlayerTime> sorted = new ArrayList<>(record.freshes);
            sorted.sort((a, b) -> Double.compare(a.time, b.time));
            for (PhantomConfig.PlayerTime pt : sorted) {
    ...
    private static void parsePbCommand(int tier, String raw) {
        ...
        String[] quickParts = raw.trim().split("\\s+");
        if (quickParts.length == 1) {
            try {
                double time = Double.parseDouble(quickParts[0]);
                ...
                mc.player.sendSystemMessage(Component.literal(
                        PREFIX + "§aT" + tier + " PB set: " + PhantomConfig.formatTime(time) + " §7(no run data)"));
                return;
            } catch (NumberFormatException ignored) {
                // not a bare number — fall through to the full parse below
            }
        }
        ...
        try {
            ...
            List<PhantomConfig.PlayerTime> supplies = new ArrayList<>();
            List<PhantomConfig.PlayerTime> freshes  = new ArrayList<>();
            int supplyCount = 0;
            while (i < dateStart - 1) {
                String player = parts[i++];
                double time   = Double.parseDouble(parts[i++]);
                if (supplyCount < 6) { supplies.add(new PhantomConfig.PlayerTime(player, time)); supplyCount++; }
                else                 { freshes.add(new PhantomConfig.PlayerTime(player, time)); }
            }
            ...
            mc.player.sendSystemMessage(Component.literal(
                    PREFIX + "§aT" + tier + " PB set: "
                            + PhantomConfig.formatTime(total)));
