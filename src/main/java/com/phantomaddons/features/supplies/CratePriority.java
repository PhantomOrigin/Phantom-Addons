...
public final class CratePriority {
    ...
    public static void onChat(String clean) {
        ...
        if (PhantomConfig.isNoPreNotifyEnabled()) {
            NotificationHud.show("§fNo " + missingSpot + "!", 6000);
            PhantomConfig.playNotificationSound(PhantomConfig.SOUND_NO_PRE);
        }

        if (!PhantomConfig.isCratePriorityEnabled()) return;
    ...
    public static void register() {
        HudElementRegistry.attachElementBefore(VanillaHudElements.CHAT,
                ...
                (ctx, tc) -> {
                    ...
                    float cx = PhantomConfig.getCratePriorityHudX() * screenW;
                    float cy = PhantomConfig.getCratePriorityHudY() * screenH;
                    float scale = PhantomConfig.getCratePriorityHudScale();
