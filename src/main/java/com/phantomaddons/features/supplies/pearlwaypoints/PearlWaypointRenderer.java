...
import com.phantomaddons.PhantomConfig.WaypointType;
...
public final class PearlWaypointRenderer {
    ...
    public static void renderWorld(PoseStack matrices, Camera camera, float tickDelta) {
        ...
        if (PhantomConfig.isDropLocationsEnabled()) {
        ...
        if (frameShouldThrow && !prevShouldThrow) {
            PhantomConfig.NotificationSound ns = PhantomConfig.getNotificationSound(PhantomConfig.SOUND_PEARL_NOW);
            if (ns.enabled) {
                PhantomConfig.playNotificationSound(PhantomConfig.SOUND_PEARL_NOW);
            } else {
    ...
    private static int resolveColor(boolean myTarget, boolean aimed, long throwIn) {
        int rgb;
        if (aimed) rgb = (myTarget && throwIn <= 0L) ? PhantomConfig.getWpColReady()
                                                      : PhantomConfig.getWpColHovered();
        else       rgb = myTarget ? PhantomConfig.getWpColCorrect()
                                  : PhantomConfig.getWpColNormal();
    ...
    private static void drawBeaconBeam(PoseStack matrices, MultiBufferSource provider,
        ...
        float hw = 0.075f;
        int col = isTarget ? PhantomConfig.getBeaconColCorrect() : PhantomConfig.getBeaconColNormal();
        int r = (col >> 16) & 0xFF;
        int g = (col >>  8) & 0xFF;
        int b =  col        & 0xFF;
        matrices.pushPose();
    ...
    private static void drawTimer(PoseStack matrices, MultiBufferSource provider,
        ...
        double heightOff = radius + 0.1 + PhantomConfig.getPearlTimerHeight() * 4.0;
        float  scale     = 0.02f + PhantomConfig.getPearlTimerSize() * 0.5f;
