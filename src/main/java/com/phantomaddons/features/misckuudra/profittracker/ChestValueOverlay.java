...
import com.phantomaddons.features.misckuudra.profittracker.ProfitRun;
...
public final class ChestValueOverlay {
    ...
    public static void tryCommitRun(CroesusListener.ChestAnalysis a) {
        ...
        run.tier           = cachedTier;
        run.durationMs     = (long) (com.phantomaddons.features.misckuudra.splits.KuudraSplitTimer.getTotalRunTime() * 1000.0);
    ...
    public static void render(GuiGraphicsExtractor ctx, CroesusListener.ChestAnalysis a) {
        ...
        if (!PhantomConfig.isChestValueGuiEnabled()) return;
        if (a == null) return;
        ...
        var m = ctx.pose();
        m.pushMatrix();
        m.translate((int)(PhantomConfig.getChestValueHudX() * screenW),
                    (int)(PhantomConfig.getChestValueHudY() * screenH));
        m.scale(PhantomConfig.getChestValueHudScale(), PhantomConfig.getChestValueHudScale());
