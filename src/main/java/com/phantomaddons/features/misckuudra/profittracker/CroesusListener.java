...
public final class CroesusListener {
    ...
    public static boolean isUnopenedChest(ItemStack stack) {
        ...
        PhantomAddons.LOGGER.warn("[PhantomAddons] Reached end of `isUnopenedChest` without knowing if the chest can still be open. Please report this as a bug.");
        PhantomAddons.LOGGER.warn("[PhantomAddons] name = {name}, lore: {lore}");
        return false;
    }
    ...
    public static ChestAnalysis analyseChest(AbstractContainerScreen<?> screen) {
        ...
        for (int i = REWARD_SLOT_START; i <= REWARD_SLOT_END && i < slots.size(); i++) {
            ...
            if (armorId != null) {
                if (PhantomConfig.isProfitArmorSalvage()) {
                    ...
                    double petMult = PhantomConfig.getKuudraPetEssenceMultiplier();
            ...
            if (weaponId != null) {
                double price;
                if (PhantomConfig.isProfitArmorSalvage() && KuudraDrops.SALVAGEABLE_WEAPON_IDS.contains(weaponId)) {
                    int stars   = KuudraDrops.countStars(rawName);
    ...
    private static void logRerollDebug(double expectedChestValue, double kismetCost, long totalValue, boolean rerollProfit) {
        if (!PhantomConfig.isDeveloperFeaturesEnabled()) return;
    ...
    public static long calculateKeyCost(int tier) {
        ...
        boolean mage = PhantomConfig.isProfitFactionMage();
