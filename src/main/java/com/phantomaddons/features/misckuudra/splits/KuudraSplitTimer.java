...
public final class KuudraSplitTimer {
    ...
    private static final List<PhantomConfig.PlayerTime> runSupplyTimes = new ArrayList<>();
    private static final List<PhantomConfig.PlayerTime> runFreshTimes = new ArrayList<>();
    ...
    public static void register() {
        ...
        net.fabricmc.fabric.api.client.message.v1.ClientReceiveMessageEvents.GAME.register((msg, overlay) -> {
            ...
            com.phantomaddons.features.supplies.SupplyTracker.onChat(msg.getString());
            ...
            if (activeSplit == Split.SUPPLIES && !com.phantomaddons.PhantomConfig.isSupplyRecoveryMsgEnabled()) {
                ...
                if (rm.find()) {
                    ...
                    runSupplyTimes.add(new PhantomConfig.PlayerTime(player, elapsed));
                }
            }
    ...
    private static void handleFreshMessage(String text) {
        ...
        int pct = fm.group(2) != null
                ...
                : com.phantomaddons.features.build.buildprogress.BuildProgressHud.getCurrentProgress();
        runFreshTimes.add(new PhantomConfig.PlayerTime(fm.group(1), elapsed, pct));
    }
    ...
    public static void onEndStart() {
        ...
        if (runStartMs > 0) {
            ...
            PhantomConfig.recordTotalRunTime(tier, total);
            com.phantomaddons.features.misckuudra.profittracker.ProfitStore.recordSessionRunTime(tier, total);

            if (PhantomConfig.updateTotalRunPb(tier, total)) {
                ...
                for (SupplyTime st : supplyTimes)
                    record.supplies.add(new PhantomConfig.PlayerTime(st.player(), st.timeSec()));
    ...
    private static void savePbIfBetter() {
        ...
        double[] pb = PhantomConfig.getSplitPb(tier);
        ...
        if (improved) PhantomConfig.setSplitPb(tier, pb);
    }
    ...
    private static void printRunSummary() {
        ...
        for (Split s : splitsForTier(tier)) {
            PhaseResult r = results.get(s);
            if (r != null)
                send(mc, "§7  " + splitLabel(s) + ": §a" + PhantomConfig.formatTime(r.wallSec()));
        }

        double total = (runEndMs - runStartMs) / 1000.0;
        send(mc, "§fTotal: §e" + PhantomConfig.formatTime(total));
        send(mc, "§6§l-----------------");
    }
    ...
    public static List<PhantomConfig.PlayerTime> getFreshTimes()     { return Collections.unmodifiableList(runFreshTimes); }
