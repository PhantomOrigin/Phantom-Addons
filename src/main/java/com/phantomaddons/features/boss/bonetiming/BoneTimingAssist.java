package com.phantomaddons.features.boss.bonetiming;
...
public final class BoneTimingAssist {
    ...
    private static void render(GuiGraphicsExtractor ctx) {
        ...
        double distToKuudra = rayEntryDistance(eye, look, kuudraBox, DISPLAY_RANGE)
                + PhantomConfig.getBoneTimingAssistOffset();
        ...
        if (!PhantomConfig.isBoneTimingAssistEnabled() || hidden) { throwNowSoundArmed = true; return; }
        ...
        if (aimedInThrowRange) {
            if (throwNowSoundArmed) {
                PhantomConfig.playNotificationSound(PhantomConfig.SOUND_BONE_THROW_NOW);
