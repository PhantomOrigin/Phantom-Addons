...
public final class FastDpsWarning {
    ...
    public static void onDpsEnd() {
        ...
        if (PhantomConfig.isFastDpsNotifyEnabled()) {
            NotificationHud.show("§cFAST DPS!", 3000);
            PhantomConfig.playNotificationSound(PhantomConfig.SOUND_FAST_DPS);
        }

        if (!PhantomConfig.isFastDpsWarningEnabled()) return;
