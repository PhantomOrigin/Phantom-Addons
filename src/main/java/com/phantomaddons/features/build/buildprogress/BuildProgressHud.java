    ...
    public static void register() {
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            if (elapsed < BUILD_DELAY_S) {
                ...
                NotificationHud.clearCountdown();
                if (!buildStartedNotified && PhantomConfig.isBuildStartedNotifyEnabled()) {
                    buildStartedNotified = true;
                    NotificationHud.show("§eBuild Started!", 3000);
                    PhantomConfig.playNotificationSound(PhantomConfig.SOUND_BUILD_STARTED);
                }
            }
        });
        ...
        HudElementRegistry.attachElementBefore(VanillaHudElements.CHAT,
                ...
                (ctx, tc) -> {
                    ...
                    if (!PhantomConfig.isBuildProgressHudEnabled()) return;
                    ...
                    float cx = PhantomConfig.getBuildProgressHudX() * screenW;
                    float cy = PhantomConfig.getBuildProgressHudY() * screenH;
                    float scale = PhantomConfig.getBuildProgressHudScale();
                    ...
                    int total = 0, count = 0;
                    for (com.phantomaddons.features.supplies.pearlwaypoints.PearlLocation loc
                            : com.phantomaddons.features.supplies.pearlwaypoints.PearlLocation.values()) {
