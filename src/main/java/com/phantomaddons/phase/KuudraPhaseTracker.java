...
public final class KuudraPhaseTracker {
    ...
    private static void handle(String raw) {
        ...
        if (msg.contains("Kuudra") || msg.contains("Ballista") || msg.contains("supplies")
            ...
            PhantomAddons.LOGGER.info("[PhaseTracker] msg='{}'", msg);
        }
    ...
    private static void logSkipDebug(Minecraft mc) {
        if (!com.phantomaddons.PhantomConfig.isDeveloperFeaturesEnabled()) return;
        ...
        PhantomAddons.LOGGER.info("[PhaseTracker] SKIP y-check tick: {}", sb);
    }

    private static void setPhase(Phase newPhase) {
        if (newPhase == currentPhase) return;
        PhantomAddons.LOGGER.info("[PhaseTracker] phase {} -> {}", currentPhase, newPhase);
