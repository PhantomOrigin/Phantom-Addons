package com.kuudrahelper.features.misckuudra.splits;

import com.kuudrahelper.KuudraConfig;
import com.kuudrahelper.features.misckuudra.splits.KuudraSplitTimer.PhaseResult;
import com.kuudrahelper.features.misckuudra.splits.KuudraSplitTimer.Split;
import com.kuudrahelper.features.misckuudra.splits.KuudraSplitTimer.SupplyTime;
...
public final class SplitHud {
    ...
    private static void render(GuiGraphicsExtractor ctx, DeltaTracker tc) {
        if (!KuudraConfig.isSplitTimerEnabled()) return;
        ...
        float scl = KuudraConfig.getSplitHudScale();
        var matrices = ctx.pose();
        matrices.pushMatrix();
        matrices.translate((int)(KuudraConfig.getSplitHudX() * screenW), (int)(KuudraConfig.getSplitHudY() * screenH));
    ...
    private static int drawSplit(GuiGraphicsExtractor ctx, Minecraft mc, int x, int y, Split s) {
        ...
        if (s == Split.SUPPLIES && KuudraConfig.isSupplyTimesEnabled()) {
