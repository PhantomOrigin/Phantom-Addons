...
public final class ChestTracker {
    ...
    public static void init() {
        total   = KuudraConfig.getChestTotal();
        success = KuudraConfig.getChestSuccess();
        fail    = KuudraConfig.getChestFail();
    ...
    private static void handleMessage(String raw) {
        ...
        if (clean.contains(WIN)) {
            ...
            save();
            KuudraHelperMod.LOGGER.info("[ChestTracker] Win  total={} s={} f={}", total, success, fail);
        ...
        if (clean.contains(DEFEAT)) {
            ...
            total = Math.min(total + 1, 60);
            save();
            KuudraHelperMod.LOGGER.info("[ChestTracker] Fail total={} s={} f={}", total, success, fail);
            return;
        }
    ...
    private static void handlePaidChest() {
        ...
        save();
        KuudraHelperMod.LOGGER.info("[ChestTracker] Paid chest → total={} s={} f={}", total, success, fail);
    }
    ...
    private static void handleFreeChest() {
        ...
        save();
        KuudraHelperMod.LOGGER.info("[ChestTracker] Free chest → total={} s={} f={}", total, success, fail);
    }
    ...
    private static boolean shouldShowHud() {
        if (!KuudraConfig.isChestTrackerVisible()) return false;
    ...
    private static void save() {
        KuudraConfig.setChestCounts(total, success, fail);
    }
