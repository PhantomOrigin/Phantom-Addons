...
public final class ProfitHud {
    ...
    public static boolean shouldShow() {
        if (!KuudraConfig.isProfitTrackerEnabled()) return false;
        if (KuudraConfig.isProfitTrackerForced()) return true;
        ...
        if (KuudraPhaseTracker.isRunActive() && !KuudraConfig.isProfitShowDuringRun()) return false;
        return true;
    }
    ...
    private static void renderCore(GuiGraphicsExtractor ctx, Minecraft mc, int screenW, int screenH) {
        float hudX  = KuudraConfig.getProfitHudX()    * screenW;
        float hudY  = KuudraConfig.getProfitHudY()    * screenH;
        float scale = KuudraConfig.getProfitHudScale();
